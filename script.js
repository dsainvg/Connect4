...
function stopGame(){
    ...
    document.getElementById("playerTurn").textContent = "Game Over! Player "+currentPlayer+" wins!";
...
function checkResult(i,j){
    for(let vx = 0;vx<=1;vx++){
        for(let vy = 0;vy<=1;vy++){
            if(vx == 0 && vy == 0) continue;
            for(let off = -3;off<=0;off++){
                if(i+off*vx < 0 || i+off*vx > 6 || j+off*vy < 0 || j+off*vy > 5) continue;
                if(cellInfo[i][j] == cellInfo[i+off*vx][j+off*vy] && cellInfo[i][j] == cellInfo[i+(off+1)*vx][j+(off+1)*vy] && cellInfo[i][j] == cellInfo[i+(off+2)*vx][j+(off+2)*vy] && cellInfo[i][j] == cellInfo[i+(off+3)*vx][j+(off+3)*vy]){
                    document.getElementById("playerTurn").textContent = `Player ${cellInfo[i][j]} wins!`;
                    for(let k = 0;k<4;k++){
                        document.getElementById("cell"+(i+(off+k)*vx)+(j+(off+k)*vy)).classList.add("blink");
                        document.getElementById("cell"+(i+(off+k)*vx)+(j+(off+k)*vy)).textContent = "X";
                    }
                    stopGame();
                    return;
                }
...
function nextMove(e){
    ...
    if(colId != -1){
        ...
        if(gameStat) {document.getElementById("playerTurn").textContent = `Player ${currentPlayer}'s turn`;}
    } 
}